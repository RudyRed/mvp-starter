...
class Pokemon1 extends React.Component {
  ...
  changeHandler(event) {
    for(var move of this.props.moves) {
      if (move.name === event.target.value) {
        this.setState({[event.target.id]: [move.name, move]});
      }
    }
  }
  ...
  clickHandler(event) {
    ...
    if ([this.state.move1[0], this.state.move2[0], this.state.move3[0], this.state.move4[0]].some(x => x === '')) {
      alert('you must pick four moves for your Pokemon!')
  } else if ($.unique([this.state.move1[0], this.state.move2[0], this.state.move3[0], this.state.move4[0]]).length < 4) {
  ...
  render() {
    return (<div>
      <form>
        <ul id="main">
          <li>Player1 Move 1:<select id="move1" value={this.state.move1[0]} onChange={this.changeHandler.bind(this)}>
